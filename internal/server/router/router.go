...
// func SetupAuth(router *mux.Router, authHandler handlers.AuthHandlerInterface) {
...
func SetupAuth(router *mux.Router, authHandler delivery.AuthHandlerInterface) {
	authSubRouter := router.PathPrefix("/auth").Subrouter()
...
func SetupStaffPersonHandlers(router *mux.Router, staffPersonHandler handlers.StaffPersonInterface) {
	router.HandleFunc("/name/{person_id}", staffPersonHandler.GetPersonByID).Methods(http.MethodGet, http.MethodOptions).Name("StaffPersonRoute")
}
